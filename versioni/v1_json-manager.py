...
def render_preview(testo_raw):
    ...
    # Rimuove eventuali virgolette esterne se presenti nel CSV
    if testo.startswith('"') and testo.endswith('"'): 
        testo = testo[1:-1]
    
    # Sostituisce i newline letterali \n con quelli reali per Markdown
    testo = testo.replace('\\n', '\n')

    # Usa solo st.markdown per permettere a LaTeX di apparire "inline"
    # Streamlit riconosce automaticamente il contenuto tra $...$
    st.markdown(testo)
